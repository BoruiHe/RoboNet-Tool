...
class GaussianNoise(nn.Module):
    ...
    def __init__(self, sigma=0.1, is_relative_detach=True,):
        ...
        self.scale = self.sigma * x.detach() if self.is_relative_detach else self.sigma * x
        self.sampled_noise = self.noise.repeat(*x.size()).float().normal_() * scale

    def forward(self, x):
        x = x + self.sampled_noise
        return x 
...
class Generator(nn.Module):
    ...
    def forward(self, input):
        shape = input.size()
        self.noise = torch.cuda.FloatTensor(shape) if torch.cuda.is_available() else torch.FloatTensor(shape)
        torch.randn(shape, out=self.noise)
