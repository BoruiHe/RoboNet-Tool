if __name__ == '__main__':
    ...
    if os.path.exists('epoch_sawyer_wd_0.1_chosen.npy') and os.path.exists('epoch_sawyer_wd_0.01_chosen.npy'):
        pass
    else:
        for wd in weight_decay:
            ...
            np.save('epoch_sawyer_wd_{}_chosen.npy'.format(wd), np.array(ls_epoch))

    for wd in weight_decay:
        ls_epoch = np.load('epoch_sawyer_wd_{}_chosen.npy'.format(wd))
        ...
        torch.save(netG.state_dict(), 'C:\\Users\\hbrch\\Desktop\\Robonet\\netG_120_160_wd_{}.pth'.format(wd))
        tools.save_weight_of_layers(netG, 'WoL_M2_chosen_{}'.format(wd))
        x = range(1, len(imgs_1)+1)
        path = 'C:\\Users\\hbrch\\Desktop\\Robonet\\plots\\'
        suffix = ('sawyer_sq_wd_{}'.format(wd), 'sawyer_sqrt_wd_{}'.format(wd))
        ...
        for i, (v_img, state) in enumerate(summer_loader):
            ...
            # cv2.waitKey()
            cv2.imwrite('./pred_img/M2_chosen_wd_{}/{}.png'.format(wd, i), pred_img.to(dtype=torch.uint8).numpy())
    pass
