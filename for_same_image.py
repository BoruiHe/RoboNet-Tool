if __name__ == "__main__":
    ...
    batch_size = 30
    nz, ngf, nc, epoch_num = 5, 64, 3, 1000
    ...
    loss_list = []
    # training on one single image
    c_imgs, c_stats, n_imgs, n_stats = next(iter(baxter_loader))
    ...
    for epoch in range(epoch_num):
        ...
        if epoch % 100 == 99:
            for i in range(2):
                ...
                fake_p = 255 * (fake[i].cpu().detach().numpy() + 1)/2
                np.moveaxis(fake_p, 0, -1)
                fake_p = np.around(np.moveaxis(fake_p.squeeze(), 0, -1), decimals=-1).astype(int)
                imgplot = plt.imshow(fake_p.squeeze())
                ax.set_title('fake')
                # draw real(next) image
                ax = fig.add_subplot(1, 2, 2)
                real_p = 255 * (n_imgs[i].cpu().detach().numpy() + 1)/2
                real_p = np.around(real_p.squeeze(), decimals=-1).astype(int)
                imgplot = plt.imshow(real_p)
                ax.set_title('real')
                plt.savefig('{}_{}'.format(epoch,i))
                plt.show()
